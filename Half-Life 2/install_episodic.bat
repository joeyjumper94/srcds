...
:FAIL:
"%~dp0"..\steamcmd\steamcmd.exe +login %name% +force_install_dir %~dp0 +app_update 380 validate +quit
if exist "%~dp0"\steamapps\downloading\380 (
	goto :CHECK:
)
goto :END:
:CHECK:
for /F %%i in ('dir /b ""%~dp0"\steamapps\downloading\380\*.*"') do (
	goto :FAIL:
)
:END:
