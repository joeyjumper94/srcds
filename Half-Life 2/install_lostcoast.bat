...
:FAIL:
"%~dp0"..\steamcmd\steamcmd.exe +login %name% +force_install_dir %~dp0 +app_update 340 validate +quit
if exist "%~dp0"\steamapps\downloading\340 (
	goto :CHECK:
)
goto :END:
:CHECK:
for /F %%i in ('dir /b ""%~dp0"\steamapps\downloading\340\*.*"') do (
	goto :FAIL:
)
:END:
